...
- Outstanding launch-side user homework: buy defensive `.com/.ai/.dev` domains (`.org` + `.com` already live); reserve PyPI `kvwarden` with a 0.0.1 stub; reserve Twitter/X handles (npm still open too); send courtesy email to Samuel Bell; cold-outreach 5 ICPs; record 30-sec demo video.
